...
func TestListJSON(t *testing.T) {
	...
	out := cmdtest.RunCmd(t, newLeafCmd(newListCmd, svc, "json"), "primary")
...
func TestListJSONScopeWithoutValue(t *testing.T) {
	...
	keys := cmdtest.JSONKeys(t, second)
	require.ElementsMatch(t, []string{"id", "scope_type", "scope_value", "role"}, keys)
...
func TestListTable(t *testing.T) {
	svc := &fakeService{rules: seedRules()}
	out := cmdtest.RunCmd(t, newLeafCmd(newListCmd, svc, "table"), "primary")
...
func TestListPropagatesAPIError(t *testing.T) {
	...
	_, err := cmdtest.RunCmdErr(t, newLeafCmd(newListCmd, svc, "json"), "primary")
