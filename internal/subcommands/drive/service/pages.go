...
// maxFilePageSize is the Drive API's per-request cap on page size for the
// file endpoints (files.list).
const maxFilePageSize = 1000

// fetchPage fetches one page of a paginated Drive list call: page is the
...
// already clamped by pageAllBudgeted to maxFilePageSize.
type budgetedFetchPage[T any] func(page string, remaining int64) (items []T, nextPageToken string, err error)

// pageAllBudgeted pages a Drive list endpoint until maxResults items are
...
// page size value is clamped to maxFilePageSize. maxResults <= 0 means no
...
func pageAllBudgeted[T any](maxResults int64, fetch budgetedFetchPage[T]) ([]T, error) {
	...
	for page, pages := "", 0; ; pages++ {
		remaining := int64(0)
		if maxResults > 0 {
			remaining = min(maxResults-int64(len(items)), maxFilePageSize)
		}
...
// pageAll pages a Drive list endpoint until NextPageToken is empty and
// returns every item across all pages. It takes no maxResults budget — the
// endpoints paged without one (files.list without a budget, permissions.list)
// are full listings. Budgeted listing uses pageAllBudgeted instead, so its
// callers cap the total with --max.
