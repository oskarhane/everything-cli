...
func TestListAggregatesProvidersJSON(t *testing.T) {
	...
	for _, row := range rows {
		...
		assert.ElementsMatch(t, []string{"name", "provider", "identity", "is_default"}, keys,
			"JSON keys must be snake_case: name, provider, identity, is_default")
		byKey[key{row["provider"].(string), row["name"].(string)}] = row
	}
	assert.Equal(t, true, byKey[key{"google", "work"}]["is_default"],
		"the google default account must be marked")
	assert.Equal(t, false, byKey[key{"google", "personal"}]["is_default"])
	assert.Equal(t, "work@example.com", byKey[key{"google", "work"}]["identity"])
	assert.Equal(t, true, byKey[key{"linear", "work"}]["is_default"],
...
// and marks each provider's default row.
func TestListTable(t *testing.T) {
	...
	assert.Contains(t, outStr, "IS_DEFAULT")
	assert.Contains(t, outStr, "work@example.com")
	assert.Contains(t, outStr, "yes", "the default account's row must be marked")
}
