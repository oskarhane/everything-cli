...
func TestGetByID(t *testing.T) {
	...
	out := cmdtest.RunCmd(t, newLeafCmd(newGetCmd, svc, "json"), "INBOX")
...
func TestGetByNameFallsBackToList(t *testing.T) {
	...
	out := cmdtest.RunCmd(t, newLeafCmd(newGetCmd, svc, "json"), "Travel")
...
func TestGetByNameTable(t *testing.T) {
	svc := &fakeService{labels: seedLabels(), getErr: errors.New("googleapi: Error 404")}
	out := cmdtest.RunCmd(t, newLeafCmd(newGetCmd, svc, "table"), "Travel")
...
func TestGetNotFound(t *testing.T) {
	svc := &fakeService{labels: seedLabels(), getErr: errors.New("googleapi: Error 404")}
	_, err := cmdtest.RunCmdErr(t, newLeafCmd(newGetCmd, svc, "json"), "Nope")
