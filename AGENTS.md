...
- Emission points that pass through the redactor: `output.writeLine` (the single chokepoint under `Print`/`PrintJSON`/`PrintTable`/`PrintToon` and `output.Debug`) and the top-level error print — main sets `root.SilenceErrors = true` and prints via `app.PrintError`, which redacts before writing stderr; `app.NewRootCommand` sets `SilenceUsage` so runtime failures emit the error line without a usage block.
